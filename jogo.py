...
BASE_MARGIN_OF_ERROR_MICROPHONE = 1500
BASE_MARGIN_OF_ERROR_SPACE = 400
...
def get_margin_of_error(base_margin, bpm):
    # Ajusta a margem de erro com base no BPM
    adjusted_margin = base_margin / (bpm / 60)
    return adjusted_margin

def check_microphone_beat(current_time):
    global score
    margin_of_error = get_margin_of_error(BASE_MARGIN_OF_ERROR_MICROPHONE, bpm)
...
def on_space_press(event):
    ...
    current_time = time.time()
    margin_of_error = get_margin_of_error(BASE_MARGIN_OF_ERROR_SPACE, bpm)
